...
svydf1 <- svydf %>%
  mutate(art_exposure = fct_drop(fct_relevel(art_exposure, "No")),
         hiv_cascade_status = factor(ifelse(hiv_knowledge == "Yes" & art_exposure == "Yes", 
                                            "Knows status and on ART",
                                            ifelse(hiv_knowledge == "Yes" & art_exposure == "No",
                                                   "Knows status and not on ART", "Does not know status"))),
         hiv_cascade_status = replace(hiv_cascade_status, is.na(art_exposure), NA),
         parents_live_here = na_if(parents_live_here, "Missing"),
         pregever = na_if(pregever, "Prefer Not to Say"),
         bf = na_if(bf, "Prefer Not to Say"),
         freq_hiv_test = na_if(freq_hiv_test, "Prefer Not to Say"),
         rel_status2 = na_if(rel_status2, "Other")) %>%
...
svydf1a <- svydf1 %>%
  filter(!is.na(parents_live_here))

svydf2a <- svydf2 %>%
  filter(!is.na(pregever))

svydf1b <- svydf1 %>%
  filter(!is.na(rel_status2))
...
tab3 <- recap_uni_catvar_svy(svy = svydf1a, var = parents_live_here)
...
tab7 <- recap_uni_catvar_svy(svy = svydf2a, var = pregever)
...
tab20 <- recap_uni_catvar_svy(svy = svydf1b, var = rel_status2)
tab21 <- recap_uni_catvar_svy(svy = svydf1c, var = bf)
...
tab3 <- recap_bi_catvar_svy(svy = svydf1a, var = parents_live_here, byvar = hiv_knowledge)
...
tab7 <- recap_bi_catvar_svy(svy = svydf2a, var = pregever, byvar = hiv_knowledge)
...
tab19 <- recap_bi_catvar_svy(svy = svydf1b, var = rel_status2, byvar = hiv_knowledge)
tab20 <- recap_bi_catvar_svy(svy = svydf1c, var = bf, byvar = hiv_knowledge)
...
tab27 <- recap_bi_catvar_svy(svy = svydf4a, var = freq_hiv_test, byvar = hiv_knowledge)
...
tab3 <- recap_bi_catvar_svy(svy = svydf6a, var = parents_live_here, byvar = art_exposure)
...
tab7 <- recap_bi_catvar_svy(svy = svydf7a, var = pregever, byvar = art_exposure)
...
tab20 <- recap_bi_catvar_svy(svy = svydf6b, var = rel_status2, byvar = art_exposure)
tab21 <- recap_bi_catvar_svy(svy = svydf6c, var = bf, byvar = art_exposure)
